...
skip_before_action :authorize
  ...
  @prod = Product.where("homepageflag = ?", true) 
  end
 ...
 def checkout
	...
	if cartlen > 0
	 ...
	 saleid = Sale.create(customer_id: session[:user_id], saledate: Date.today, salenotes: session[:cart_prod].to_s)
	# For each item in the shopping cart save the record in the in OrderItem table
	while i < cartlen
Saledetail.create(sale_id: saleid.id, product_id: session[:cart_prod][i].to_i, saleprice: Product.find(session[:cart_prod][i].to_i).saleprice, prodqty: session[:cart_qty][i])
		i = i + 1
	end 
  	# Set session cart arrays to nil. 
  ...
  @results = 1
  @searchresults = Product.where("description like ? OR productname like ?", "%" + @searchinput.to_s + "%", "%" + @searchinput.to_s + "%")
  end
  @image = Product.all
  if productimage = "1"
  puts "Hello!"
